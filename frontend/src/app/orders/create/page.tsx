...
import { OrderCreate, OrderStatus, OrderItemCreate } from '@/types/order'
...
export default function CreateOrderPage() {
  ...
  if (loading) return <div className="container mx-auto p-6">Loading data...</div>
            ...
            >
              ...
              {items.map(item => (
                <option key={item.id} value={item.id}>
                  {item.name} (¥{item.price.toLocaleString()}) - 在庫: {item.stock}
                </option>
              ))}
            </select>
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {selectedItems.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center">商品が選択されていません</TableCell>
                </TableRow>
              ) : (
                selectedItems.map((selectedItem, index) => {
                  ...
                  return (
                    <TableRow key={index}>
                      ...
                      <TableCell>¥{selectedItem.price.toLocaleString()}</TableCell>
                      <TableCell>
                        <Input
                          type="number"
                          min="1"
              ...
              <TableRow>
                ...
                <TableCell colSpan={2} className="font-bold">¥{calculateTotal().toLocaleString()}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </div>
